...
query= (f'from(bucket:\"{bucket}\") \
    ...
    |> drop(columns:["year","weekday_name","weekday_no","pce","pce_alias","month","month_short","_measurement","type", "month_name","mois_num"]) \
    |> max()')
    ...
    if float(cell.row) > 2 : # on passe la ligne des titres + la premiere ligne ( T° systematiquement absente du fichier d'extraction )
        ...
        if str(firstWorksheet.cell(cell.row, column=8).value) == "Mesuré" :
            ...
            if date_time_obj > lastdate:
                ...
                firstWorksheet.cell(cell.row, column=20).value = compodate.month # mois chiffre d.strftime('%m')
                firstWorksheet.cell(cell.row, column=12).value = compodate.strftime('%m') # mois avec le 0 
